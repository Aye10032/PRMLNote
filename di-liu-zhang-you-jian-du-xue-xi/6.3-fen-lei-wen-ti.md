...
$$
\begin{align}
\min \textcolor{red}{E}&=\min -\ln P(\mathbf y^1,\dots,\mathbf y^N\vert\mathbf w_1,\dots,\mathbf w_K,\mathbf x^1,\dots,\mathbf x^N) \nonumber
\\
...
\\
 (x\vert y = 0) &\sim N(\mathbf\mu_0,\mathbf\Sigma)
 \\
  (x\vert y = 1) &\sim N(\mathbf\mu_1,\mathbf\Sigma)
\end{align}
$$
则对数似然函数为：
$$
